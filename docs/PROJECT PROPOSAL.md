**Tên dự án:** RideStream - Nền tảng Dữ liệu Thời gian thực (Real-time CDC & Analytics Platform) 
**Lĩnh vực:** Data Engineering / Big Data Processing / Cloud Computing

## 1. Đặt vấn đề (Problem Statement)

Trong các hệ thống di chuyển đô thị hiện đại (Ride-hailing), dữ liệu được sinh ra liên tục với khối lượng khổng lồ. Thách thức lớn nhất đối với các kỹ sư dữ liệu là làm sao thu thập sự thay đổi trạng thái của các cuốc xe theo thời gian thực (Real-time) từ cơ sở dữ liệu vận hành (OLTP) mà không làm ảnh hưởng đến hiệu năng hệ thống, đồng thời kết hợp chúng với dữ liệu bối cảnh (thời tiết, giao thông) để đưa ra các quyết định kinh doanh như định giá động (Dynamic Pricing) với độ trễ thấp nhất.

Dự án **RideStream** được đề xuất nhằm giải quyết bài toán trên bằng cách xây dựng một kiến trúc luồng dữ liệu đầu cuối (End-to-End Data Pipeline) áp dụng kỹ thuật Change Data Capture (CDC).
...
- **Lớp Lưu trữ Đám mây (Cloud Data Platform):** Dữ liệu thô và đã xử lý được lưu trữ trên **Azure Data Lake Storage Gen2 (ADLS Gen2)**, sau đó nạp vào **Azure Synapse Analytics** (hoặc Google BigQuery) để làm Data Warehouse.
